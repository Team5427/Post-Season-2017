...
public class PIDStraightMovement extends PIDCommand
{
	...
	/**
	 * The SpeedControllers that contain the motors that are not controlled by
	 * the PIDController within this command.
	 */
	...
	/**
	 * The maximum speed the PIDController can output to the motors on the
	 * robot.
	 */
	double maximumSpeed;
	...
	/*
	 * Command implemented from PIDCommand This is called automatically after
	 * the constructor of the command is run. We only use this to start the
	...
	/**
	 * Command implemented from PIDCommand This returns the value to be used by
	 * the PID loop. We are returning the angle received from the NavX on the
	 * robot.
	 */
	@Override
	...
	/**
	 * Command implemented from PIDCommand This is sent the output from the PID
	 * loop for us to use. We are setting the side of the robot that we control
	 * in this PID loop to the output to maintain the angle of 0 to go straight.
	 * After we reach maximum power, we activate the PIDDistance command in
	 * order to finish traveling a certain distance.
	 * 
	 * @param output
	 *            the output of the PIDController within PIDStraightMovement
	 *            when given our current Yaw.
	 */
	@Override
	...
	/**
	 * Command implemented from PIDCommand. When this returns true, the command
	 * runs end() Our method returns true if the robot has traveled close enough
	 * to the certain distance, with our tolerance.
	 * 
	...
	/**
	 * Command implemented from PIDCommand This is called whenever this command
	 * is interrupted by something else. We do not use this for anything
	 * purposeful, so we just call end().
	 */
	@Override
	 ...
	 * isFinished() returns true or interrupted() runs. We use this to end the
	 * PID loop, reset the encoders' distances, and set the speed of our
	 * SpeedController.
	 */
	@Override
	public void end()
	...
	/**
	 * Command implemented from PIDCommand This is called manually and is meant
	 * to disable the PID loop and reset values. We are using this to disable
	 * the PID loop, reset the PID loop and the encoders' distances, and set the
	 * speed of the side of the robot that we control to 0;
	 */
	@Override
	public void free()
	{
